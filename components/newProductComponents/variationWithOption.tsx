...
const variationOptionResult = (props, ref) => {
    ...
    const setVariation = (varIndx, optnIndx, price) => {
        var newPrice = { 'Price': price, "status": "available" };
        props.setVariations(varIndx, optnIndx, newPrice);
    }
        ...
        <div className="grid grid-cols-8 pb-6 sm:w-11/12">
            ...
            <div className="col-span-7">
                ...
                <table className="text-sm border text-gray-500 text-center w-full">
                    <thead>
                        <tr>
                            <th className="border w-3/12 text-base font-normal p-4">{props.variationName}</th>
                            {variationOption ?
                                <th className="border w-5/12 text-base font-normal p-4">{props.variationOptionName}</th>
                                : null}
                            <th className="border w-2/12 text-base font-normal p-4">Price</th>
                        </tr>
                    </thead>
                    <tbody>
                        {props.variationLists.map((item, index) => {
                            return <tr key={index}>
                                <td className="border-b text-center">{`${item}`}</td>
                                {variationOption ?
                                    <td>
                                    ...
                                    : null}
                                {variationOption ? <td>
                                    <table className="w-full border m-0 p-0 text-gray-800 text-center">
                                        <tbody >
                                            {props.variationOptionList.map((optn, indx) => {
                                                return <tr key={indx}><td className="border-b py-2"><input
                                                    id={`PRICE-${index}${indx}`}
                                                    onKeyDown={blockInvalidChar}
                                                    onChange={(e) => setVariation(index, optn, e.target.value)}
                                                    type="number" min="0" max="10000"
                                                    pattern="[0-9]*"
                                                    placeholder={`enter price here.`}
                                                    className="price w-full input-appearance text-center text-red-500 font-semibold placeholder-red-500::placeholder" /></td></tr>
                                            })}
                                        </tbody>
                                    </table>
                                </td> : null}
                                ...
                                {variationOption ? null : <td className="border text-center p-2">
                                    <input
                                        ...
                                        type="number" min="0" max="10000"
                                        pattern="[0-9]*"
                                        className="input-appearance text-center text-grey-darker py-2" />
                                </td>}
                            </tr>
                        })}
